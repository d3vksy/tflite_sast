...
def _parse_operator(buf, tbl):
    ...
    # field 4: builtin_options (UOffset → WhileOptions 테이블)
    # While 연산자(BuiltinOperator 코드 82)의 WhileOptions: field 0=cond_subgraph_index(i32), field 1=body_subgraph_index(i32)
    if op.builtin_options_type == 119:  # BuiltinOptions.WhileOptions union tag
        try:
...
def compute_risk_score(findings):
    ...
    C(M) = (2*n_C + n_H) / (2*N)   (N >= 1), 0 (N = 0)
           Critical=rank2, High=rank1, Medium=rank0 기반 집중도 지수
    R(M) = log2(1 + W(M)) * (1 + C(M))
           N=0이면 R(M)=0
    ...
    # 등급 서열 기반 집중도 지수 C(M) ∈ [0, 1]
    # Critical=rank2, High=rank1, Medium=rank0
    C = (2 * n_C + n_H) / (2 * N)
